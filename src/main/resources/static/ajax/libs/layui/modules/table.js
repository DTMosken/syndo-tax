...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports){
  ...
  ,parseTempData = function(item3, content, tplData, text){ //表头数据、原始内容、表体数据、是否只返回文本
  	// ykc add value
	tplData['value'] = content;
    var str = item3.templet ? function(){
      return typeof item3.templet === 'function' 
        ? item3.templet(tplData, content)
  ...
  ,TPL_MAIN = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}{{# if(d.data.className){ }} {{ d.data.className }}{{# } }}" lay-filter="LAY-table-{{d.index}}" lay-id="{{ d.data.id }}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">'
    ...
    ,'<div class="layui-table-page">'
      // ykc add style="display: inline-block;"
      ,'<div id="layui-table-page{{d.index}}" style="display: inline-block;"></div>'
      // ykc add toolbar
	  ,'<div class="layui-table-page-toolbar layui-hide-xs" style="display: inline-block;position: absolute; right: 20px;"></div>'
    ,'</div>'
    ,'{{# } }}'
  ...
  Class.prototype.renderToolbar = function(){
    ...
    // ykc add page toolbar
	var laybt = {
		filter: {title: '筛选列', layEvent: 'laytable_filter', icon: 'layui-icon-cols'},
	...
	if(typeof options.bottomToolbar === 'object') {
		layui.each(options.bottomToolbar, function(i, item){
			var thisItem = typeof item === 'string' ? laybt[item] : item;
			if(thisItem){
				laybtElem.push('<div class="layui-inline" title="'+ thisItem.title +'" style="width: 30px; height: 30px; line-height: 30px; border-radius: 3px; border: 1px solid #ccc; text-align: center; cursor: pointer; margin-left: 10px;" '
	...
	$('.layui-table-page-toolbar').html(laybtElem.join(''));
  }
  ...
  Class.prototype.events = function(){
    ...
    // ykc add page toolbar event
	that.layPage.on('click', '*[lay-event]', function(e) {
		var othis = $(this);
		var events = othis.attr('lay-event');
		var openPanel = function(sets) {
		  var list = $(sets.list)
		  ,panel = $('<ul class="layui-table-tool-panel" style="bottom: 40px; right: 20px; position: fixed; top: auto; left: auto;"></ul>');
		  panel.html(list);
		  //限制最大高度
		  if(options.height){
			panel.css('max-height', options.height - (that.layTool.outerHeight() || 50));
		...
		switch(events){
		  ...
		  case 'laytable_print': //打印
			...
			,style = ['<style>'
			  ,'body{font-size: 12px; color: #666;}'
			  ,'table{width: 100%; border-collapse: collapse; border-spacing: 0;}'
			  ,'th,td{min-width: 30px;word-wrap:break-word;word-break: break-all;line-height: 20px; padding: 9px 15px; border: 1px solid #ccc; text-align: left; font-size: 12px; color: #666;}'
			  ,'a{color: #666; text-decoration:none;}'
			  ,'*.layui-hide{display: none}'
			,'</style>'].join('')
			,html = $(that.layHeader.html()); //输出表头
		...
		layui.event.call(this, MOD_NAME, 'bottomToolbar('+ filter +')', $.extend({
		  event: events
		  ,config: options
		},{}));
	});
