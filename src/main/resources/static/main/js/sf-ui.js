    ...
    $.extend({
    	...
    	table: {
            ...
            init: function(options) {
            	var defaults = {
        		    ...
        		    onEditableSave: undefined
            	};
            	options = $.extend(defaults, options);
            	
            	/**表格没有设置宽度时，根据列标题自适应 开始 gxg**/
            	if (!!options.columnWidthAuto)
                options.columns.map(function(column, index) {
					if (!!column.title && !column.width) {
						var width = column.title.length * 22;
						if (width < 80) {
							width = 80;
						}
                ...
                $('#' + options.id).bootstrapTable({
                    ...
                    resizeMode: $.table.resizeMode || 'overflow'
                });
			},
            ...
            onLoad: function(data){
				//设置本页记录数
        		table.options.pageRows = data.rows.length;
        		table.options.totalRows = data.total;
            },
            onPageChange: function(pageNumber) {
            	table.options.pageNumber = pageNumber;
            },
            // 当所有数据被加载时触发
            ...
            search: function(formId, tableId, data) {
            	...
            	params.queryParams = function(_params) {
                    ...
                    search.pageSize = _params.limit;
                    search.pageNum = _params.offset / _params.limit + 1;
                    search.searchValue = _params.search;
                    search.orderByColumn = _params.sort;
                    search.isAsc = _params.order;
    		        return search;
    		    }
    		    if($.common.isNotEmpty(tableId)){
            ...
            getSelections: function() {
            	return $('#' + $.table._option.id).bootstrapTable('getSelections');
            },
            // 查询表格首列值
    	...
    	form: {
    		...
    		reset: function(formId, tableId) {
            	...
            	// 如果存在多个部门树，那么这个方法就不对了，后面需更改（可采用class名称判断）
            	if (_form.find('#treeId').length > 0) {
            		$('#treeId').val('');
            	}
            	// 多选下拉重置
            	if (_form.find('.selectpicker').length > 0) {
    	...
    	modal: {
            ...
            openOptions: function (options) {
                ...
                if ($.common.isEmpty(options.yes)) {
                	options.yes = function(_index, layero) {
                    	options.callBack(_index, layero);
                    }
                }
                var btnCallback = {};
            ...
            openFull: function (title, url, btn, width, height) {
                ...
                var index = layer.open({
            		...
            		yes: function(_index, layero) {
            	        var iframeWin = layero.find('iframe')[0];
            	        iframeWin.contentWindow.submitHandler(_index, layero);
            	    },
            	    cancel: function() {
            	        return true;
            	    }
        ...
        operate: {
            ...
            addFull: function(id, btn) {
                ...
                $.modal.openFull("添加" + table.options.modalName, url, btn);
            },
            // 添加访问地址
            addUrl: function(id) {
            ...
            editFull: function(id, btn) {
            	...
            	if ($.common.isNotEmpty(id)) {
            		...
            		if(table.options.type == table_type.bootstrapTreeTable) {
            			...
            			var _row = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId);
                	    url = table.options.updateUrl.replace("{id}", _row);
            		}
            	}
            	$.modal.openFull("修改" + table.options.modalName, url, btn);
            },
            // 修改访问地址
            editUrl: function(id) {
            	var url; // = "/404.html";
            ...
            ajaxSuccess: function (result) {
            	if (result.code == web_status.SUCCESS && table.options.type == table_type.bootstrapTable) {
                	...
                	$.modal.msgSuccess(result.msg);
                	$.treeTable.refresh();
                } else if (result.code == web_status.SUCCESS && table.options.type == table_type.layuiTable) {
                	$.modal.msgSuccess(result.msg);
                	$.table.refresh();
            ...
            successCallback: function(result) {
                if (result.code == web_status.SUCCESS) {
                    ...
                    } else if (parent.table.options.type == table_type.layuiTable) {
                        $.modal.close();
                        parent.$.modal.msgSuccess(result.msg);
                        parent.$.table.refresh();
        ...
        tree: {
        	...
        	init: function(options) {
        		...
        		// 树结构初始化加载
        		var setting = {
					async: $.extend({enable: false}, options.async),
    				callback: $.extend({
        		...
        		if(setting.async.enable) {
					...
					var tree = $.fn.zTree.init($("#" + options.id), setting, []);
					$._tree = tree;
				} else {
