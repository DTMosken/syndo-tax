...
$(function() {
    ...
    let resizer_bar_width;
    // 菜单切换
    $('.navbar-minimalize').click(function() {
    	if (isMobile) {
            $("body").toggleClass("canvas-menu");
            $("#resizerBar").addClass("resizerBarDisplay")
        } else {
            let $resizerBar = $("#resizerBar");
            let $page_wrapper = $('#page-wrapper');
            if(!$resizerBar.hasClass("resizerBarDisplay")) {
                resizer_bar_width = $resizerBar.parent().css('width')
            }
            $("body").toggleClass("mini-navbar");
            $resizerBar.toggleClass("resizerBarDisplay")
            if($resizerBar.hasClass("resizerBarDisplay")) {
                $resizerBar.parent().css('width','')
                $page_wrapper.css('margin-left','')
            }else{
                if(resizer_bar_width !== undefined) {
                    $resizerBar.parent().css('width', resizer_bar_width)
                    $page_wrapper.css('margin-left', resizer_bar_width)
                }
            }
        }
        SmoothlyMenu();
    ...
    //左侧菜单拖动改变宽度
    var el = document.getElementById('resizerBar')
    //初始化参数
    var $el = $(el),
        $pel = $el.parent(),
        $rightEl = $('#page-wrapper'),
        initWidth = 0,
        //鼠标的 X 和 Y 轴坐标
        x = 0;
    $el.mousedown(function(e){
        ...
        x = e.clientX;
        initWidth = $el.parent().width();

        //绑定事件
        $(document).bind("mousemove",mouseMove).bind("mouseup click",mouseUp)
    ...
    function mouseMove(e){
        var wid = initWidth + e.clientX - x;
        $pel.width(wid);
        $rightEl.css('margin-left', wid);
    }
    //停止事件
    function mouseUp(){
        $('iframe').css('pointer-events','auto')
        $(document).unbind("mousemove", mouseMove).unbind("mouseup click", mouseUp)
    }
...
$(function() {
    ...
    // 点击关闭按钮，关闭选项卡
    $('.menuTabs').on('click', '.menuTab .fa-times-circle', closeTab);
    ...
    function tabCloseAll() {
        var isFixed = $('.page-tabs-content').find('.active').hasClass("fixed");
